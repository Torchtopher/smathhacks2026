...
DEFAULT_FAKE_GPS_JITTER_METERS = float(os.getenv("FAKE_GPS_JITTER_METERS", "12.0"))
...
def apply_deterministic_gps_jitter(
    coords: list[float],
    *,
    boat_id: str,
    max_jitter_meters: float,
) -> list[float]:
    if max_jitter_meters <= 0:
        return coords

    lat, lon = float(coords[0]), float(coords[1])
    digest = hashlib.sha256(boat_id.encode("utf-8")).digest()

    # Stable pseudo-random radius/angle derived from boat_id.
    radius_unit = int.from_bytes(digest[:8], "big") / float(2**64 - 1)
    angle_unit = int.from_bytes(digest[8:16], "big") / float(2**64 - 1)
    radius_m = max_jitter_meters * math.sqrt(radius_unit)
    angle_rad = angle_unit * 2.0 * math.pi
    ...
    meters_per_degree_lon = meters_per_degree_lat * max(
        0.01, abs(math.cos(math.radians(lat)))
    )
    lat_jittered = lat + (north_m / meters_per_degree_lat)
    lon_jittered = lon + (east_m / meters_per_degree_lon)
...
def send_backend_report(
    *,
    ...
    timestamp = float(frame.get("unix_time_s") or time.time())
    payload = {
        "boat_id": boat_id,
        "timestamp": timestamp,
        "gps_lat": float(gps[0]),
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Register a new backend boat and bypass boat_id file cache for this run. "
            "Useful when you do not want to reuse or overwrite cached IDs."
        ),
    )
    parser.add_argument(
        "--boat-id-file",
    ...
    parser.add_argument(
        "--gps-jitter-meters",
        type=float,
        default=DEFAULT_FAKE_GPS_JITTER_METERS,
        help=(
            "Max deterministic per-boat GPS jitter radius in meters to reduce overlap "
            f"(default: {DEFAULT_FAKE_GPS_JITTER_METERS}). Set 0 to disable."
        ),
    )
    parser.add_argument(
        "--loop",
...
def run_once(
    ...
    if args.backend_base:
        ...
        effective_boat_id = args.boat_id or backend_boat_id
        backend_boat_id, backend_registration = ensure_boat_id(
            backend_base=args.backend_base,
            explicit_boat_id=effective_boat_id,
            boat_id_file=Path(args.boat_id_file),
            boat_name=args.boat_name,
            weight_class=args.weight_class,
            use_cache=not args.new_boat,
            persist_cache=not args.new_boat,
        )
        report_gps = apply_deterministic_gps_jitter(
            synthetic_gps["coords"],
            boat_id=backend_boat_id,
            max_jitter_meters=max(args.gps_jitter_meters, 0.0),
        )
        if report_gps != synthetic_gps["coords"]:
            synthetic_gps["coords"] = report_gps
            synthetic_gps["jitter_meters_max"] = max(args.gps_jitter_meters, 0.0)
            synthetic_gps["jitter_note"] = "Deterministic per-boat offset applied."
